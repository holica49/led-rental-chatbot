...
#### 지원하는 자연어 패턴
- **날짜**: "내일", "모레", "다음 주 화요일", "이번주 금요일", "12월 25일"
- **시간**: "오후 3시", "14:30", "아침", "점심 시간"
- **참석자**: "김대리와", "박과장, 최팀장과"
- **장소**: "강남 스타벅스에서", "2층 회의실에서", "줌으로"
- **우선순위**: "중요한", "긴급", "간단한"
...
🤖 AI 분석 결과:
📊 신뢰도: 92%
🔍 파싱 정보: 날짜: 2025-08-22, 시간: 10:00, 제목: Q4 결산 브리핑, 장소: 강남역 스타벅스, 참석자: 박팀장, 최대리, 회의 유형: internal, 우선순위: high, 알림: 15분 전
...
|--------|------|------|------|
| **이름** | Title | 사용자 이름 | ✅ |
| **LINE WORKS ID** | Text | LINE WORKS 사용자 ID | ✅ |
| **이메일** | Email | 이메일 주소 | ✅ |
| **부서** | Select | 부서명 | ✅ |
| **직급** | Select | 직급 | ✅ |
| **표시명** | Text | 표시용 이름 | ⚪ |
...
2. 제목: **"LED 렌탈 사용자 관리"**
3. [위 스키마](#-사용자-관리-데이터베이스-신규)에 따라 속성 생성
...
### 🔥 LINE WORKS 스마트 일정 등록
다양한 자연어 패턴으로 정확한 일정 생성:

```
기본: "내일 오후 2시 회의"
고급: "다음 주 화요일 오후 3시에 강남 스타벅스에서 김대리와 중요한 프로젝트 회의, 30분 전 알림, PPT 준비"
```
...
### 🆕 사용자 관리 시스템 오류
- **사용자 정보 없음**: `NOTION_USER_DATABASE_ID` 확인
- **미등록 사용자**: 대시보드에서 사용자 등록 필요
- **이메일 매핑 실패**: 사용자 DB의 이름/이메일 정보 확인

### 🆕 자연어 파싱 오류
- **날짜 계산 틀림**: 요일 계산 로직 확인 (이번주/다음주)
- **신뢰도 낮음**: 더 구체적인 표현으로 입력
- **참석자 인식 안됨**: 사용자 DB에 해당 이름 등록 확인
